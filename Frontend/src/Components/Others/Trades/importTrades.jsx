...
function ImportPreviewModal({ isOpen, onClose, summary }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/55 px-4">
      ...
      >
        <h2 id="import-summary-title" className="text-2xl font-semibold text-text-primary">
          Import Complete
        </h2>
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div className="rounded-panel border border-state-success/30 bg-state-success/10 p-4">
            ...
            <p className="mt-1 text-2xl font-semibold text-state-success">{summary.successCount}</p>
          </div>
          <div className="rounded-panel border border-state-danger/30 bg-state-danger/10 p-4">
            ...
            <p className="mt-1 text-2xl font-semibold text-state-danger">{summary.failedCount}</p>
          </div>
        </div>

        <div className="mt-5">
          <h3 className="text-body-lg font-semibold text-text-primary">Trade preview (up to 5)</h3>
          ...
          {summary.previewTrades.length > 0 ? (
            <div className="mt-3 overflow-hidden rounded-md border border-border">
              <table className="min-w-full divide-y divide-border text-left">
                ...
                <tbody className="divide-y divide-border bg-white text-body">
            ...
            <p className="mt-2 text-body text-text-secondary">No preview trades were returned by the API.</p>
          )}
        </div>

        <div className="mt-6 flex justify-end">
          <button type="button" onClick={onClose} className="ui-btn-primary py-2">
            OK
          </button>
        </div>
      </div>
    ...
    <>
      <form onSubmit={handleSubmit} className="mx-auto w-full max-w-[980px] space-y-6 pb-10">
        <section className="rounded-panel border border-border bg-surface-card p-6">
          <p className="text-body text-text-secondary">{instructions}</p>
          ...
          <div
            ...
            onDrop={handleDrop}
          >
            <span className="mx-auto inline-flex h-14 w-14 items-center justify-center rounded-full bg-brand-900/10 text-brand-900">
              <Upload size={24} />
            </span>

            <p className="mt-5 text-body-lg font-semibold text-text-primary">Drag and drop your CSV file here</p>
            <p className="mt-1 text-body text-text-secondary">or click to browse</p>
            ...
            <button
              ...
              className="ui-btn-primary mt-4 py-2"
            >
              Choose File
            </button>

            {selectedFile ? (
              <p className="mt-4 inline-flex items-center gap-2 text-body text-state-success">
                <CheckCircle2 size={16} /> {selectedFile.name}
              </p>
            ) : null}

            {validationError ? (
              <p className="mt-4 inline-flex items-center gap-2 text-body text-state-danger">
                <XCircle size={16} /> {validationError}
              </p>
            ) : null}

            {!validationError && error ? (
              <p className="mt-4 text-body text-state-danger">
        ...
        <section className="rounded-panel border border-border bg-surface-card p-6">
          <h2 className="text-card-title font-semibold">File Requirements</h2>
          <ul className="mt-4 space-y-2 text-body text-text-secondary">
            <li className="inline-flex items-center gap-2">
              <FileText size={16} /> CSV files only
            </li>
            <li className="inline-flex items-center gap-2">
              <FileText size={16} /> Upload one file only
            </li>
          </ul>
        </section>
        ...
        <section className="flex items-center justify-between rounded-panel border border-border bg-surface-card px-6 py-5">
          <div className="inline-flex items-center gap-3">
            <span className="inline-flex h-12 w-12 items-center justify-center rounded-md bg-brand-900 text-text-inverse">
              <Download size={18} />
            </span>

            <div>
              <h2 className="text-card-title font-semibold">Download CSV Template</h2>
              <p className="text-body text-text-secondary">Not sure about the format? Download our sample template.</p>
            </div>
          </div>
          ...
          <a
            ...
            download
            className="rounded-pill border border-brand-900 px-5 py-2 text-body font-medium text-brand-900 hover:bg-brand-50"
          >
            Download
          </a>
        </section>
        ...
        <div className="flex items-center justify-between">
          ...
          <button type="submit" className="ui-btn-primary py-2 disabled:opacity-70" disabled={loading}>
