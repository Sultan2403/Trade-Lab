...
const sideNavItems = [
  { id: "account-management", label: "Account Management", icon: Settings2 },
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (activeAccount?.id) setPendingAccountId(activeAccount.id);
  }, [activeAccount?.id]);

  const switchableAccounts = useMemo(() => {
    if (accounts.length === 0) return activeAccount ? [activeAccount] : [];
    const hasActive = accounts.some(
      (account) => account.id === activeAccount?.id,
    );
    return hasActive || !activeAccount
      ? accounts
  ...
  return (
    <div className="flex min-h-screen bg-surface-base">
      {/* ---------------- Sidebar ---------------- */}
      <aside className="w-64 border-r border-border bg-surface-card p-4 flex flex-col gap-4">
        <h2 className="text-xl font-semibold text-text-primary mb-4">
          Settings
        </h2>
        {sideNavItems.map(({ id, label, icon, badge }) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 p-2 rounded-md transition-colors ${
                isActive
                  ? "bg-surface-selected text-brand-900"
            ...
            >
              ...
              <span>{label}</span>
              {badge && (
                <span className="ml-auto rounded bg-[#DBEFF2] px-2 py-0.5 text-caption font-medium text-brand-900">
                  {badge}
                </span>
              )}
            </Link>
      ...
      <main className="flex-1 p-6 space-y-8">
        ...
        {profileError && (
          <Alert severity="error">
            Unable to load account information. Please refresh.
          </Alert>
        )}
        ...
        {profileLoading && !activeAccount ? (
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-panel border border-border bg-surface-card p-6">
              ...
              <div className="mt-4 flex flex-wrap justify-between gap-4 border-b border-border pb-5">
                <div className="space-y-2">
                  <h4 className="text-3xl font-semibold text-text-primary">
                    {activeAccount.name}
                  </h4>
              ...
              <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3 text-center">
                <div>
              ...
              <div className="mt-6 space-y-3">
                ...
                <button
                  ...
                  disabled={
                    !pendingAccountId || pendingAccountId === activeAccount.id
                  }
                >
                  Switch Account
                </button>
            ...
            <div className="rounded-panel border border-border bg-surface-card p-6 flex flex-wrap justify-between items-center gap-4">
              <div>
                <h3 className="text-2xl font-semibold text-text-primary">
                  Manage All Accounts
                </h3>
                <p className="mt-1 text-body text-text-secondary">
                  Add new accounts, edit account details, and manage your
                  portfolio
                </p>
              </div>
              <Link
        ...
        {/* Nested content for future sections */}
        <Outlet />
      </main>
    </div>
  );
