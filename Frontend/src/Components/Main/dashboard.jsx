    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-2xl font-semibold text-text-primary">{account.name}</h3>
            {isSelected && (
          ...
          {account.type && (
            <span className={`inline-flex rounded-md px-2.5 py-1 text-caption font-medium ${typeStyle}`}>
              {account.type}
            </span>
          )}
        </div>
      ...
      <div className="mt-5 grid gap-3 text-text-secondary">
        <div>
          ...
          <p className="text-2xl font-semibold text-text-primary">${formatCurrency(account.starting_balance)}</p>
        </div>

        <div>
          <p className="text-body">Current Balance</p>
          <p className="text-4xl font-semibold text-text-primary">${formatCurrency(account.current_balance)}</p>
        </div>
...
export default function Dashboard() {
  ...
  const activeAccount = useMemo(() => {
    const preselected = accounts.find((account) => account.id === selectedAccountId);
  ...
  return (
    <div className="space-y-6">
      {error && (
        <Alert severity="error">Unable to load your accounts right now. Please try again.</Alert>
      )}
        ...
        <>
          {activeAccount && (
            <section className="space-y-3">
              <h2 className="text-xl font-semibold text-text-primary">Selected Account</h2>
          ...
          {remainingAccounts.length > 0 && (
            <section className="space-y-3">
              <h2 className="text-xl font-semibold text-text-primary">Other Accounts</h2>
